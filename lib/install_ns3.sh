...
cd "$install_dir/ns-$ns3_ver/" || exit 1
./waf clean
./waf configure --python=/usr/bin/python3 --build-profile=debug --enable-examples --enable-tests -d debug --with-pybindgen="$allinone_dir/pybindgen"  --with-nsclick="$allinone_dir/click" --with-brite="$allinone_dir/BRITE"
